---
title: Analyse direct infusion RF Lens scans
author: "Jonas D. Förster"
project:
  execute-dir: file
params:
  rawfile: 'OE2-20220111_011'
...
```{r}
ramp_name <- "Funnel RF Level"
ramp_axis_name <- "RF Lens (%)"

ramp_axis_label_every_nth <- 1
```
...
opt_picked_df <-
  ...
  ungroup() %>%
  select(-pre_intensity_mean, rf_lens = !!ramp_symbol)
...
opt_df <-
  ...
  select(-eval_group) %>%
  mutate(rf_lens = as.character(rf_lens)) %>%
  replace_na(list(rf_lens = "weak")) %>%
  mutate(rf_lens_rawfile = rawfile_name)

opt_df

write_csv(opt_df, paste0(rawfile_name, "_rf_lens_opt.csv"))
```
...
gen_precursor_ramp_intensity_plot <- function(sel_trans_df, sel_pre_df) {
  opt_val <- inner_join(opt_picked_df, sel_pre_df, by = join_by(eval_group, seq_mod, pre_charge))$rf_lens
  opt_val_plot <- match(opt_val, ramp_vals)
  ...
  # ensure that all ramp values are present
  pre_mean_sd_df <- tibble(!!ramp_symbol := ramp_vals, n_ramp_val = unclass(factor(!!ramp_symbol, levels = ramp_vals)), pre_intensity_mean = 0, pre_intensity_sd = 0)
  ...
  plot <-
    ...
    mutate(
      n_ramp_val = unclass(factor(!!ramp_symbol, levels = ramp_vals)),
      iondesc = factor(iondesc, levels = iondesc_plotorder),
      border = if_else(iondesc %in% label_other, 0.01, NA_real_)
    ) %>%
    ggplot(aes(x = n_ramp_val, y = trans_intensity_mean_per_step)) +
  ...
  if (!is_empty(opt_val)) {
    ...
    plot <- plot + annotate("segment", x = opt_val_plot, xend = opt_val_plot, y = -Inf, yend = 0, linewidth = marker_width, color = "red", linetype = marker_linetype)
  }
  ...
  plot <-
    ...
    coord_cartesian(xlim = c(1, length(ramp_vals)), ylim = c(0, NA), default = TRUE) +
    labs(
      x = ramp_axis_name,
      y = "Intensity (a.u.)",
      fill = "Transition",
