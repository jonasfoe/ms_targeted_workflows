---
title: Make inclusion lists for direct infusion RF Lens scans
...
Make inclusion lists for per-precursor RF Lens scans
...
rf_scan <- c(10, 25, 30, 40, 50, 65, 80, 100, 125, 150)
rf_scan
...
```{r}
rf_scan_full_schuffled <-
  map(1:scan_count, ~ sample(rf_scan)) %>%
  flatten_dbl()
...
rf_scan_full_schuffled %>%
  ...
  scale_y_reverse() +
  labs(x = "RF Lens (%)", y = "Scan Order")
```

```{r}
out_dir <- "static_rf_scan"
...
optimizations_df <-
  ...
  mutate(across(c(nce_top_i, nce_seq_cov), weak_to_na)) %>%
  mutate(across(c(nce_top_i, nce_seq_cov), as.numeric)) %>%
...
schedule_df <-
  ...
  replace_na(list(nce = nce_default))
...
schedule_out_df <-
  schedule_out_df %>%
  mutate(`RF Lens (%)` = list(rf_scan_full_schuffled)) %>%
  unchop(`RF Lens (%)`) %>%
...
schedule_out_df %>%
  ...
  group_walk(function(df, n) {
    write_csv(df, file.path(out_dir, paste0(peptide_set, "_rf_shuffled_", n$n_list, ".csv")))
  })
```
